...
unsigned int thoi1,coub,tinh1;

unsigned long tinh2;


unsigned int mang[70],thoi;

unsigned char vao[420],mang1[70];
...
P2OUT=0xf0;
  ...
  while(1){
  	for(nut=0;nut<420;nut++) vao[nut]=0;
  	       lcd_clear();
           lcd_puts("do cong huong");
           _delay_cycles(12000000);  
       for(nut=380;nut<420;nut++){
  	       lcd_clear();
           lcd_puts("tanso");
           chu=nut/10;
           docgiatri2(chu);
           ...
           ghikt(chu);
            ...
            chu=0;
           while((thoi1<30)){
                 if(((P4IN&0x01)==0)&&(chu==0)){
                 susa++;   chu=1;          }
              if(((P4IN&0x01)!=0)&&(chu!=0)){
                 susa++;   chu=1;          }
                   
                   if( susa>20)  {vao[nut]=1;
                   	               lcd_puts("=>xac lap");
         ...
         thoatnan:
           ...
           lcd_puts("do");
            ...
            thoi1=0;
             tinh1=TA0R; 
             while((P4IN&0x07)==0x07);
             tinh2=TA0R;
             tinh2=tinh2+thoi1*60000/nut-tinh1;
             docgiatri(tinh2);
           ...
           TA0CCR0 =237;
         for(nut=200;nut<420;nut++){   
           if(vao[nut]==1){
           lcd_clear();
           lcd_puts("tan so cong huong");
           chu=nut/10;
           docgiatri2(chu);
           ghikt('.');
           chu=nut%10+'0';
          ...
          _delay_cycles(6000000);  
            ...
            for(coub=0;coub<70;coub++){ mang1[coub]=0; mang[coub]=0;}
           thoi=0; 
           coub=0;
           thoi=60000/nut;
            TA0CCR0 =60000/nut;
           while((thoi1<14));
             ...
             chu=P4IN&0x07;
             while(thoi1<60){
           if((P4IN&0x07)!=chu){chu=P4IN&0x07;
           	                    mang[coub]=14*thoi+(thoi1-14)*237;
           	                    mang1[coub]=chu; coub++; 
           	                    if(coub>70) break;
           	                    }
          ...
          for(coub=0;coub<70;coub++){
          	if(mang[coub]!=0){
          		 lcd_clear();
          		docgiatri1(mang[coub]);
          		  _delay_cycles(6000); 
          		     lcd_puts("trangthai");
          		       _delay_cycles(6000); 
          		       ghilenh(0xc0);
          		         _delay_cycles(6000); 
          		         docgiatri2(mang1[coub]);
